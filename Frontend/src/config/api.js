...
export const API_ENDPOINTS = {
  ...
  deleteChatbotFile: (chatbotId, filename) =>
    `${BASE_URL}/api/cms/files/${chatbotId}/${encodeURIComponent(filename)}`,
  


  deleteChatbot: (chatbotId) => `${BASE_URL}/api/cms/delete_chatbot/${chatbotId}`,
  
  // Widget/Public endpoints - προσθήκη /api/public prefix
  calendarAuth: (apiKey) => `${BASE_URL}/api/public/calendar-auth/${apiKey}`,
  availableSlots: (apiKey, date) => `${BASE_URL}/api/public/available-slots/${apiKey}?date=${date}`,
};
