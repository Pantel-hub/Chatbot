...
export default function FormSteps({
	...
	useEffect(() => {
		...
		if (formData.botTypePreset) {
			...
			setFormData((prev) => ({
				...
				greeting: t(
					`botTypeDefaults.${formData.botTypePreset}.greeting`
				),
				personaSelect: t(
					`botTypeDefaults.${formData.botTypePreset}.persona`,
					{ lng: "en" }
				),
			}));
		...
		const loadExistingFiles = async (chatbotId) => {
			try {
				const res = await fetch(
					API_ENDPOINTS.getChatbotFiles(chatbotId),
					{ credentials: "include" }
				);
	...
	const handleDeleteExistingFile = useCallback(
		async (filename) => {
			...
			// επιβεβαίωση χρήστη
			if (!window.confirm(`Να διαγραφεί το αρχείο "${filename}" ;`))
				return;

			try {
				const res = await fetch(
					API_ENDPOINTS.deleteChatbotFile(
						editMode.chatbot_id,
						filename
					),
	...
	const handleNextGuarded = () => {
		...
		if (currentPage === 3) {
			...
			if (
				...
				!settings?.booking_page_url?.trim()
			) {
				alert(
					"Παρακαλώ εισάγετε το Google Booking Page URL για να συνεχίσετε."
				);
				return;
			}
		}
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			formDataToSend.append(
				"company_info",
				JSON.stringify(formDataWithFaq)
			);
		...
		<>
			...
			<div className="mb-6 sm:mb-8">
				...
				<div className="flex justify-end gap-2 sm:gap-3 mb-3 sm:mb-4 items-center">
					<img
						...
						className={`w-6 sm:w-7 h-4 sm:h-5 cursor-pointer rounded shadow hover:opacity-80 transition-opacity ${
							i18n.language === "en"
				...
				<p
					className="text-xs sm:text-sm font-medium text-indigo-600"
					aria-live="polite"
					aria-atomic="true"
				>
					{currentPage + 1 + ". "}
			...
			<form onSubmit={handleSubmit}>
				<div className="min-h-[450px]">
					...
					{currentPage === 1 && (
						<UploadFiles {...commonProps} ref={uploadFilesRef} />
					)}
				...
				<div
					className={`flex flex-col sm:flex-row gap-2 sm:gap-0 pt-6 ${
						currentPage > 0
							? "sm:justify-between"
							: "sm:justify-end"
					}`}
