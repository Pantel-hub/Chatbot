...
export default function Capabilities({
	...
	const checkboxLabel =
		"text-sm sm:text-base text-slate-700 leading-tight break-words";
	const cardClasses =
		"rounded-2xl border border-slate-200 bg-white p-4 md:p-6 shadow-sm";
	...
	const coreFeatures = useMemo(
		() => [
			{ key: "leadCapture", label: t("features.leadCaptureForms") },
			{
				...
				label: t("features.appointmentScheduling"),
			},
			{
				key: "feedbackCollection",
				label: t("features.feedbackCollection"),
			},
		],
		[t]
	);
			...
			<div className="space-y-6 md:space-y-8">
				...
				{appointmentEnabled && (
					<section
						...
						aria-hidden={!appointmentEnabled}
					>
						...
						<label className="flex flex-col gap-1">
							...
							<select
								...
								disabled={disabled}
							>
								<option value="bot_managed">
									Το bot (Πλήρης διαχείριση)
								</option>
								<option value="user_managed">
									Η σελίδα κράτησης μου (Google)
								</option>
							</select>
						</label>
							...
							<div className="space-y-3">
								<label className="flex flex-col gap-1">
									...
									<small className="text-xs text-slate-500">
										💡 Αντιγράψτε το URL από "Κοινή χρήση
										σελίδας κράτησης" στο Google Calendar
									</small>
								</label>
							</div>
						)}
							...
							<div className="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-3">
								...
								<label className="flex flex-col gap-1">
									...
									<input
										...
										className="rounded-lg border border-slate-300 px-3 py-2 text-sm"
										value={Number(
											settings.maxAppointmentsPerSlot ?? 1
										)}
										onChange={(e) =>
											updateSetting(
				...
				{leadEnabled && (
					...
					>
						<h4 className={groupTitle}>
							{t("capabilities.leadCaptureFields")}
						</h4>
						<div className="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
