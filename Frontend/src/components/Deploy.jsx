...
export default function Deploy({
	...
	const botId =
		apiKey || formData.widgetId || formData.botId || "your-bot-id";
	...
	const [domains, setDomains] = useState(
		Array.from(new Set([domainInitial].filter(Boolean)))
	);
	...
	const startCalendarOAuth = async () => {
		try {
			...
			const res = await fetch(
				`${BASE_URL}/api/public/calendar-auth/${apiKey}`,
				{
					credentials: "include",
					signal: controller.signal,
				}
			...
			const data = await res.json();
			if (data?.auth_url)
				window.open(data.auth_url, "gcal", "width=600,height=700");
	...
	const handlePublish = async () => {
		...
		if (onPublish) {
			try {
				await onPublish({ botId, domains, config: configObj });
			} catch {}
		}
	};
				...
				<div className="flex items-center gap-3">
					<span
						className={`inline-flex items-center gap-2 text-sm ${
							published ? "text-green-600" : "text-slate-600"
						}`}
					>
						<span
					...
					<button
						...
						style={{
							backgroundColor: published ? "#16a34a" : undefined,
						}}
					>
					...
					<div className="rounded-xl border border-slate-200 p-4">
						...
						<div className="rounded-xl border border-slate-200 p-4">
							...
							<p className="text-xs text-slate-600 mb-3">
								Συνδέστε το Google Calendar (μία φορά). Μετά το
								bot θα κλείνει ραντεβού αυτόματα.
							</p>
							...
							{calStatus === "connected" && (
								<div className="mt-4 pt-4 border-t border-slate-200">
									...
									<p className="text-xs text-slate-500 mt-2">
										Τα ραντεβού θα εμφανίζονται στο
										ημερολόγιο "Bot_Bookings"
									</p>
								</div>
							)}
						</div>
