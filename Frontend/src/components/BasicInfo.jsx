...
export default function BasicInfo({
  ...
  editWebsiteData,  
  onWebsiteDataChange,  
  isEditingWebsiteData,  
  setIsEditingWebsiteData,  
  isRescraping,  
  setIsRescraping,
}) {
      ...
      <div className="space-y-6 sm:space-y-8">
            ...
            <div className="md:col-span-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className={`${baseInputClasses} ${errors.contactEmail ? errorClasses : normalClasses}`}
                  placeholder="info@company.gr"
                  required
                  disabled={disabled}
                />
              </div>
              {errors.contactEmail && <small className={errorTextClasses}>{t('required')}</small>}
            </div>
            {/* 🆕 ΤΕΛΟΣ CONTACT EMAIL */}
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
            ...
            <div>
              <label htmlFor="allowedDomains" className={labelClasses}>
                {t('domain')}
                {errors.allowedDomains ? requiredMark : null}
              </label>
              <div className="relative">
                <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-2 sm:pl-3">
                  <GlobeAltIcon className="h-5 w-5 text-slate-400" />
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className={`${baseInputClasses} ${errors.allowedDomains ? errorClasses : normalClasses}`}
              ...
              {errors.allowedDomains && <small className={errorTextClasses}>{t('required')}</small>}
            </div>
            

            {editMode && editWebsiteData && (
              <div className="md:col-span-2 mt-4">
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                  <h4 className="text-sm font-semibold text-slate-800 mb-3">
                    {t('websiteDataOptions', 'Website Data Options')}
                  </h4>
                  ...
                  <div className="space-y-3 mb-4">
                    <label className="flex items-center space-x-3 cursor-pointer">
                      <input
                        type="radio"
                        name="websiteDataOption"
                        checked={!isEditingWebsiteData && !isRescraping}
                        onChange={() => {
                          setIsEditingWebsiteData(false);
                          setIsRescraping(false);
                        }}
                        className="w-4 h-4 text-indigo-600"
                      />
                      <span className="text-sm text-slate-700">
                        {t('keepExistingData', 'Keep existing website data')}
                      </span>
                    </label>
                    ...
                    <label className="flex items-center space-x-3 cursor-pointer">
                      <input
                        type="radio"
                        name="websiteDataOption"
                        checked={isEditingWebsiteData}
                        onChange={() => {
                          setIsEditingWebsiteData(true);
                          setIsRescraping(false);
                        }}
                        className="w-4 h-4 text-indigo-600"
                      />
                      <span className="text-sm text-slate-700">
                        {t('editWebsiteData', 'Edit website data manually')}
                      </span>
                    </label>
                    ...
                    <label className="flex items-center space-x-3 cursor-pointer">
                      ...
                      />
                      <span className="text-sm text-slate-700">
                        {t('rescrapeWebsite', 'Re-scrape website (get fresh data)')}
                      </span>
                    </label>
                  </div>
                  
                  {/* Textarea για manual edit */}
                  {isEditingWebsiteData && (
                    <div className="mt-4">
                      <label className="block text-xs sm:text-sm font-medium text-slate-600 mb-2">
                        {t('websiteDataContent', 'Website Content')}
                      </label>
                      <textarea
                        value={editWebsiteData}
                        onChange={(e) => onWebsiteDataChange(e.target.value)}
                        className="w-full p-3 border border-slate-200 rounded-xl bg-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 resize-none text-sm font-mono"
                        rows={12}
                        placeholder={t('editWebsiteDataPlaceholder', 'Edit the scraped website content here...')}
                      />
                      <p className="mt-2 text-xs text-slate-500">
                        📊 {editWebsiteData.length.toLocaleString()} characters
                      </p>
                    </div>
                  )}
                  ...
                  {isRescraping && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      <p className="text-sm text-blue-800">
                        ⚠️ {t('rescrapingWarning', 'The website will be scraped again when you update the chatbot. This may take a few moments.')}
                      </p>
                    </div>
                  )}

                </div>  {/* ✅ Κλείνει το Website Data Options container */}
              </div>  
            )} {/* ✅ Κλείνει το conditional block */}
          </div>  {/* ✅ Κλείνει το grid */}
        </div>  {/* ✅ Κλείνει το Web Presence section */}
