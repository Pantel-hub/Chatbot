...
export default function Test({
	formData,
	chatbotId,
	serverUrl = API_ENDPOINTS.chat,
}) {
	const { t } = useTranslation();

	const primary = formData?.primaryColor || "#4f46e5";
	const botName =
		(formData?.botName || "").trim() || t("test.defaults.botName");
	const greetingRaw =
		(formData?.greeting || "").trim() || t("test.defaults.greeting");
	...
	const sendToServer = async (text) => {
		...
		try {
									...
									prev.map((msg) => {
										if (msg.id === botMessageId) {
											...
											const displayText = newText
												...
												.trim();
											const hasAction =
												/<\s*action\b[^>]*>[\s\S]*?<\s*\/\s*action\s*>/i.test(
													newText
												);
											return {
												...msg,
			...
			setMessages((prev) => [
				...prev,
				{ from: "bot", text: t("test.errors.network") },
			]);
		} finally {
			setIsTyping(false);
		}
	...
	const parseActionBlocks = (fullText) => {
		const actionMatch = fullText.match(
			/<\s*action\b[^>]*>([\s\S]*?)<\s*\/\s*action\s*>/i
		);
		if (actionMatch) {
			try {
				const actionData = JSON.parse(actionMatch[1]);
				console.log(
					"🔍 ACTION detected in test mode:",
					actionData.type
				);
		...
		<div className="rounded-2xl border border-slate-200 bg-white p-4 md:p-6 shadow-sm">
			...
			<div className="rounded-xl border border-slate-200 bg-slate-50 p-4 sm:p-5">
					...
					<div className="mb-4 flex flex-col items-center text-center">
						<div className="mb-3 grid h-10 w-10 sm:h-12 sm:w-12 place-items-center rounded-full bg-slate-200 text-slate-500">
							...
							>
								<path
									fill="currentColor"
									d="M12 2a1 1 0 011 1v1h3a2 2 0 012 2v1h1a1 1 0 110 2h-1v5a4 4 0 01-4 4h-6a4 4 0 01-4-4V9H5a1 1 0 110-2h1V6a2 2 0 012-2h3V3a1 1 0 011-1zM7 13a1 1 0 100 2h1a1 1 0 100-2H7zm9 0h-1a1 1 0 100 2h1a1 1 0 100-2z"
								/>
							</svg>
						</div>
				...
				{suggestions.length > 0 && (
					<>
						<div className="mb-2 text-center text-xs sm:text-sm font-medium text-slate-600">
							{t("test.suggestions.title")}
						</div>
										...
										<div className="flex items-start gap-2">
											...
											<div className="flex-1">
												...
												<div className="text-amber-800 leading-relaxed">
													Οι φόρμες επικοινωνίας και
													ραντεβού δεν είναι
													διαθέσιμες εδώ. Δοκιμάστε το
													widget στην ιστοσελίδα σας.
												</div>
											</div>
										</div>
									</div>
											...
											components={{
												...
												code: ({
													...
													inline ? (
														...
														<code
															className="block bg-slate-200 p-2 rounded my-2 text-xs"
															{...props}
														/>
													),
												strong: ({
			...
			<div className="mt-4 rounded-xl border border-indigo-100 bg-indigo-50 p-3 sm:p-4 text-xs sm:text-sm text-slate-700">
				<span className="font-semibold">{t("test.infoBox.title")}</span>{" "}
				{t("test.infoBox.body")}
			</div>
