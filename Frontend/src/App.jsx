...
export default function App() {
  ...
  const performRealSubmit = async (finalData) => {
    ...
    try {
      ...
      interval = setInterval(() => {
        setFormProgress((p) => (p === null ? 8 : Math.min(p + Math.random() * 8, 92)));
      }, 700);
      ...
      console.log('%c[App.jsx] ✅ performRealSubmit -> New chatbot created', 'color:green; font-weight:bold;');
      console.log('Previous activeChatbotId:', activeChatbotId);
      console.log('New chatbot_id from backend:', responseData.chatbot_id);
      
      setApiKey(responseData.api_key);
      setWidgetScript(responseData.widget_script);
      setActiveChatbotId(responseData.chatbot_id);

      console.log('%c[App.jsx] 🔄 activeChatbotId updated!', 'color:orange; font-weight:bold;', responseData.chatbot_id);
  ...
  const performUpdate = async (finalData, chatbotId) => {
    ...
    try {
      ...
      const res = await fetch(`${API_ENDPOINTS.updateChatbot}/${chatbotId}`, {
        method: 'PUT',
        credentials: 'include',
        body: finalData,
      });
      ...
      console.log('%c[App.jsx] ✏️ performUpdate -> Updating existing chatbot', 'color:deepskyblue; font-weight:bold;');
      ...
      setTimeout(() => {
        ...
        console.log('%c[App.jsx] 🔄 activeChatbotId updated after edit!', 'color:orange; font-weight:bold;', chatbotId);
  ...
  const handleEditBot = (chatbotId) => {
    ...
    console.log('%c[App.jsx] 🛠 handleEditBot called', 'color:purple; font-weight:bold;');
    console.log('Selected chatbotId from Dashboard:', chatbotId);
    console.log('Previous activeChatbotId:', activeChatbotId);
    
    setEditingChatbot({ chatbot_id: chatbotId });

    console.log('%c[App.jsx] 🔄 activeChatbotId set via handleEditBot', 'color:orange; font-weight:bold;', chatbotId);
    ...
    <div className="min-h-screen font-sans">
      ...
      {currentView === 'dashboard' && (
        <div className="min-h-screen bg-gradient-to-b from-white to-zinc-50">
          ...
          onSelectBot={(id) => {
            console.log('%c[App.jsx] 🎯 onSelectBot called', 'color:orange; font-weight:bold;');
            console.log('Previous activeChatbotId:', activeChatbotId);
            console.log('New selected bot ID from Dashboard:', id);

            setActiveChatbotId(id);

            console.log('%c[App.jsx] ✅ activeChatbotId updated via Dashboard click', 'color:green; font-weight:bold;', id);
            }}
          />
        </div>
      )}
                    ...
                    {formSubmitted ? (
                      ...
                      <div className="w-full pt-4 lg:pt-0">
                        <FormSteps
                          ...
                          setApiKey={setApiKey}
                          setWidgetScript={setWidgetScript}
                        />
                      </div>
                    )}
                  </div>
