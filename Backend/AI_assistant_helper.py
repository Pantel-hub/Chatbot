...
def process_knowledge_blocking(
    ...
    try:
        ...
        upload_file_ids = [] #λίστα με τα αναγνωριστικά ids των αρχείων το χρειαζεται η openai για το indexing
        ...
        for file_info in all_file_paths:
            ...
            try:
                ...
                openai_file_ids_map[filename_key] = uploaded_file.id #αφου έχει ανεβει και έχει αποκτήσει id
                
                upload_file_ids.append(uploaded_file.id)
                logger.info(f"File uploaded to OpenAI: {filename_key} -> {uploaded_file.id}")
        ...
        if not upload_file_ids:
            logger.warning("⚠️ No files were successfully uploaded to add to Vector Store")
            return {
                "vector_store_id": vector_store.id,
                "openai_file_ids": {},
        ...
        raise KnowledgeProcessingError(f"Failed to process knowledge data with OpenAI: {e}")
        ...
        if temp_website_file_path and os.path.exists(temp_website_file_path):
            try:
                ...
                logger.info(f"✅ Deleted temporary website file: {temp_website_file_path}")
            except Exception as e:
                logger.warning(f"⚠️ Failed to delete temporary file {temp_website_file_path}: {e}")
...
def update_vector_store_blocking(
    ...
    try:
        ...
        if update_website and website_data:
            ...
            if "website_data" in existing_file_ids:
                ...
                try:
                    ...
                    logger.info(f"🗑️ Deleted old website file from Vector Store: {old_file_id}")
                    
                    # Διαγραφή από OpenAI
                    openai_client.files.delete(old_file_id)
                    logger.info(f"🗑️ Deleted old website file from OpenAI: {old_file_id}")
                except Exception as e:
        ...
        if update_faq and faq_text:
            ...
            if "faq_data" in existing_file_ids:
                ...
                try:
                    ...
                    openai_client.beta.vector_stores.files.delete(
                        vector_store_id=vector_store_id,
                        file_id=old_faq_file_id
                    )
                    logger.info(f"🗑️ Deleted old FAQ file from Vector Store: {old_faq_file_id}")

                    openai_client.files.delete(old_faq_file_id)
                    logger.info(f"🗑️ Deleted old FAQ file from OpenAI: {old_faq_file_id}")
                except Exception as e:
            ...
            with open(temp_faq_file_path, "rb") as f:
                new_faq_file = openai_client.files.create(
                    file=f,
                    purpose="assistants"
                )
            ...
            logger.warning("⚠️ update_faq=True αλλά δεν δόθηκε faq_text — skip FAQ update.")
        ...
        if local_file_paths: 
            ...
            for file_info in local_file_paths:
                ...
                if filename_key in ("website_data", "faq_data"):
                    logger.info(f"⏭️ Skipping {filename_key} — already handled separately.")
                    continue
        ...
        return {
            "vector_store_id": vector_store_id,
            "openai_file_ids": updated_file_ids
        }
...
def delete_file_from_vector_store_blocking(
    vector_store_id: str,
    file_id: str
) -> bool:
    """
...
async def add_message_to_thread(
    ...
    try:
        ...
        logger.info(f"✅ Added {role} message to thread {thread_id}: {thread_message.id}")
